...
faqs:
 ...
 a: "The real rate of return is the net investment growth after subtracting the erosion caused by inflation (r_{real} = frac(1 + Return){1 + Inflation} - 1). It represents your true gain in purchasing power."
  - q: "Why is life expectancy critical in corpus planning?"
 a: "Life expectancy determines your retirement duration (N_{post} = Life Expectancy - Retirement Age). Underestimating your life expectancy risks outliving your retirement savings."
...
1. **Current Age & Retirement Age (N_{pre})**: Determines the accumulation horizon (N_{pre} = Retirement Age - Current Age).
2. **Retirement Age & Life Expectancy (N_{post})**: Determines the drawdown duration (N_{post} = Life Expectancy - Retirement Age).
...
4. **Post-Retirement Return (R_{post})**: The annual interest or growth rate earned on your remaining corpus during retirement.
...
Retire Monthly Expense = Current Expense × (1 + f)^N_{pre}

First Year Annual Expense  (W_1) = Retire Monthly Expense × 12

#### Step 2: Real Rate of Return (r_{real})

r_{real} = frac(1 + R_{post)}{1 + f} - 1

#### Step 3: Required Retirement Corpus Goal (NPV of Drawdown Stream)

The calculator models an annuity-due stream where expenses grow annually with inflation (f) while remaining capital earns return (R_{post}). The initial corpus required equals the net present value (NPV) of all N_{post} annual withdrawals:

Target Corpus = sum_{t=1}^N_{post} W_1 × (1 + r_{real})^-(t-1)
...
1. **Pre-Retirement Horizon**: 60 - 30 = 30 years.
2. **Retirement Duration**: 85 - 60 = 25 years.
3. **Monthly Expense at Age 60**:
 Monthly Expense = 5,000 × (1 + 0.06)^30 = 5,000 × 5.74349 ≈ \28,717.46 4. **First Year Annual Expense (W_1)**: \28,717.46 × 12 = $344,609.55
5. **Real Return Rate**:
 r_{real} = (1 + 0.08 / 1 + 0.06) - 1 = 1.88679%
6. **Required Target Corpus**:
 Target Corpus = sum_{t=1}^25 344,609.55 × (1 + 0.0188679)^-(t-1) ≈ \5,800,866 
...
* **Q2: What happens if post-retirement return equals inflation?**
  * A2: When return matches inflation (r_{real} = 0), the corpus needed simplifies to First Year Annual Expense multiplied by the number of retirement years.
