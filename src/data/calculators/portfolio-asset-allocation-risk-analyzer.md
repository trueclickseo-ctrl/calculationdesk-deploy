...
faqs:
  - q: "How does this calculator compute portfolio volatility?"
    a: "The calculator computes portfolio volatility using Modern Portfolio Theory variance equations: $\sigma_p = \sqrt{\sum \sum w_i w_j \sigma_i \sigma_j \rho_{ij}}$. It incorporates standard correlation coefficients ($\rho$) between asset classes (e.g. -0.10 correlation between Equities and Bonds)."
  ...
  - q: "How are portfolio risk classifications determined?"
    a: "Risk levels are classified based on computed portfolio volatility ($\sigma_p$): Conservative (<5%), Moderate-Conservative (5%–9.9%), Moderate-Aggressive (10%–14.9%), and Aggressive ($\ge$15%)."
references:
...
1. **Asset Values ($V_i$)**: The dollar amount allocated to each of the 5 asset classes (Equities, Bonds, Cash, Real Estate, Alternatives). Total value $V_{\text{total}} = \sum V_i$.
2. **Expected Returns ($E(R_i)$)**: Estimated annual growth rate per asset class.
3. **Asset Volatility ($\sigma_i$)**: Annualized standard deviation of returns per asset class.
...
$$E(R_p) = \sum_{i=1}^{5} w_i \times E(R_i)$$

#### 3. Portfolio Volatility ($\sigma_p$) & Correlation Matrix

Unlike simple weighted averages, true portfolio volatility depends on how asset prices move together (correlation $\rho_{i,j}$):

$$\sigma_p = \sqrt{\sum_{i=1}^{5} \sum_{j=1}^{5} w_i \times w_j \times \sigma_i \times \sigma_j \times \rho_{i,j}}$$
...
- **Equities ($V_1 = \$60,000$)**: $w_1 = 0.60$, $E(R_1) = 10\%$, $\sigma_1 = 15\%$
- **Bonds ($V_2 = \$30,000$)**: $w_2 = 0.30$, $E(R_2) = 5\%$, $\sigma_2 = 5\%$
- **Cash ($V_3 = \$10,000$)**: $w_3 = 0.10$, $E(R_3) = 3\%$, $\sigma_3 = 1\%$

#### Step 1: Expected Portfolio Return
$$E(R_p) = (0.60 \times 10\%) + (0.30 \times 5\%) + (0.10 \times 3\%) = 6.0\% + 1.5\% + 0.3\% = \mathbf{7.80\%}$$

#### Step 2: Portfolio Variance Computation
$$\text{Variance} = w_1^2 \sigma_1^2 + w_2^2 \sigma_2^2 + w_3^2 \sigma_3^2 + 2 w_1 w_2 \sigma_1 \sigma_2 \rho_{1,2} + 2 w_1 w_3 \sigma_1 \sigma_3 \rho_{1,3} + 2 w_2 w_3 \sigma_2 \sigma_3 \rho_{2,3}$$

$$\text{Variance} = (0.60^2 \times 0.15^2) + (0.30^2 \times 0.05^2) + (0.10^2 \times 0.01^2) + 2(0.60 \times 0.30 \times 0.15 \times 0.05 \times -0.10) + \dots$$

$$\text{Variance} = 0.0081 + 0.000225 + 0.000001 - 0.00027 = \mathbf{0.008233}$$

$$\sigma_p = \sqrt{0.008233} \approx \mathbf{9.07\%}$$
...
* **Q1: What does portfolio risk classification mean?**
  * A1: Risk classifications categorize portfolios based on expected price fluctuation ($\sigma_p$). Lower volatility (<5%) represents conservative preservation; higher volatility ($\ge$15%) represents aggressive equity-heavy allocation.
