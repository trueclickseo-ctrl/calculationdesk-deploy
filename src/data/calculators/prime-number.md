...
If N is composite, it can be written as a × b = N. If both factors were strictly greater than sqrt(N), then a × b > sqrt(N) × sqrt(N) = N, which is impossible. Therefore, every composite number must have at least one prime factor less than or equal to sqrt(N).

* *Example (N = 29)*: sqrt(29) ≈ 5.38. We only need to check prime divisors 2, 3, 5$. Since 29 is not divisible by 2, 3, or 5, it is **prime**.
...
1. **Trial Division Check**:
 - If N le 1: Not prime.
 - If N = 2 or N = 3: Prime.
 - If N ±od 2 = 0 or N ±od 3 = 0: Composite.
 - Tests 6k ± 1$ prime steps up to sqrt(N).
2. **Prime Factorization**: For composite numbers, repeatedly divides out prime factors (2, 3, 5...) to generate exponent notation (e.g., 18 = 2^1 × 3^2$).
...
- **Prime Factorization**: 100 = 2 × 2 × 5 × 5 = 2^2 × 5^2$.
