'use client';
import React, { useState } from 'react';
import { RefreshCw } from 'lucide-react';

export const seoData = {
  whatIs: 'Reduce complex fractions to simplest integers fractions ratios.',
  formula: 'Division of both elements by Greatest Common Divisor (GCD).',
  example: '8/12 simplifies to 2/3.',
  faqs: [{ q: 'What is a simplified fraction?', a: 'A fraction where numerator and denominator share no common factors other than 1.' }]
};

export default function SimplifyFractions() {
  const [num, setNum] = useState<number>(8);
  const [den, setDen] = useState<number>(12);
  const [result, setResult] = useState<any>(null);

  const handleReset = () => { setNum(8); setDen(12); setResult(null); };
  const gcd = (a: number, b: number): number => b ? gcd(b, a % b) : a;

  const handleCalculate = () => {
    const divisor = gcd(Math.abs(num), Math.abs(den));
    setResult({
      n: num / divisor,
      d: den / divisor
    });
  };

  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-5 rounded-2xl border border-border bg-card p-6 shadow-sm">
          <h2 className="text-xl font-bold text-foreground mb-6">Fraction Values</h2>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-foreground/80 mb-2">Numerator</label>
              <input type="number" value={num} onChange={e => setNum(Number(e.target.value))} className="block w-full py-3 px-4 text-sm font-medium rounded-xl border border-border bg-background outline-none focus:border-primary text-foreground/80" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-foreground/80 mb-2">Denominator</label>
              <input type="number" value={den} onChange={e => setDen(Number(e.target.value))} className="block w-full py-3 px-4 text-sm font-medium rounded-xl border border-border bg-background outline-none focus:border-primary text-foreground/80" />
            </div>
            <div className="flex gap-3 pt-4 border-t border-border">
              <button type="button" onClick={handleCalculate} className="flex-1 bg-primary text-white font-bold py-3 px-4 rounded-xl hover:bg-primary-hover shadow-lg shadow-primary/20 transition-all cursor-pointer text-sm">Solve</button>
              <button type="button" onClick={handleReset} className="border border-border hover:bg-border font-semibold py-3 px-4 rounded-xl text-foreground/70 transition-all cursor-pointer text-sm"><RefreshCw className="h-4 w-4" /></button>
            </div>
          </div>
        </div>
        <div className="lg:col-span-7 rounded-2xl border border-border bg-card p-6 shadow-sm">
          <h2 className="text-xl font-bold text-foreground mb-6">Simplified Fraction</h2>
          {result ? (
            <div className="rounded-xl bg-primary/5 p-6 border border-primary/10 text-center">
              <span className="block text-xs font-bold text-foreground/50 uppercase font-mono">Reduced Form</span>
              <span className="block text-4xl font-extrabold text-primary mt-2">
                {result.n} / {result.d}
              </span>
            </div>
          ) : (
            <div className="flex h-64 items-center justify-center text-foreground/40 text-sm">Enter fraction values.</div>
          )}
        </div>
      </div>
    </div>
  );
}