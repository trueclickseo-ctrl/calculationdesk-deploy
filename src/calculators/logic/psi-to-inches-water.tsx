...
export const seoData = {
  whatIs: 'The PSI to Inches of Water Converter translates pressure from pounds per square inch to inches of water column.',
  formula: '$$\\text{Inches of Water} = \\text{PSI} \\times 27.6799$$',
...
export default function Converterpsitoincheswater() {
  const [val, setVal] = useState<number>(1);
  const [result, setResult] = useState<number | null>(null);

  const handleReset = () => { setVal(1); setResult(null); };
  const handleCalculate = () => {
    setResult(val * 27.679904);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-5 rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-foreground/80 mb-2">Pressure in PSI</label>
        ...
        <div className="lg:col-span-7 rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          {result !== null ? (
            <div className="rounded-xl bg-primary/5 p-6 border border-primary/10 text-center">
              ...
              <span className="block text-4xl font-extrabold text-primary mt-2">{result.toFixed(3)} Inches of Water (inH2O)</span>
            </div>
          ) : (
