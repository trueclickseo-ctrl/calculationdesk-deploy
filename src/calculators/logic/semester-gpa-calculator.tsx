...
export default function SemesterGpaCalculator() {
  ...
  const calculate = () => {
    ...
    if (g.length !== c.length || g.length === 0) return;
    const totalCredits = c.reduce((a, b) => a + b, 0);
    if (totalCredits === 0) return;
    const totalPoints = g.reduce((acc, gp, i) => acc + gp * c[i], 0);
  ...
  return (
    <div className="rounded-3xl border border-border bg-card p-6 md:p-8 shadow-sm">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-lg font-bold text-foreground">Semester GPA Calculator</h2>
      </div>
      <div className="space-y-4">
        ...
        <button onClick={calculate} className="w-full py-3 bg-primary text-white font-bold rounded-xl">Calculate Semester GPA</button>
        {result !== null && <p className="text-sm font-bold text-primary text-center">Semester GPA: {result}</p>}
      </div>
    </div>
  );
}
