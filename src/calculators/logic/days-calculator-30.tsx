...
export const seoData = {
  whatIs: `The 30 Day Calculator computes the exact calendar date 30 days in the future or past from any starting date.`,
  formula: `Date offsets are calculated by adding or subtracting days from a start timestamp:

$$\\text{Future Date} = \\text{Start Date} + 30 \\quad (\\text{days})$$
$$\\text{Past Date} = \\text{Start Date} - 30 \\quad (\\text{days})$$`,
  example: `**Find the date 30 days after January 1st:**
- Kidding/Future Date = **January 31st**.`,
  faqs: [
    {
    ...
    {
      q: 'What is a 30-day offset typically used for?',
      a: 'It is commonly used to track payment due dates, trial subscription expirations, standard return windows, and invoice net-30 terms.',
    },
  ],
};

export default function Days30Calculator() {
  ...
  const handleCalculate = () => {
    ...
    future.setDate(start.getDate() + 30);
          ...
          {result ? (
            <div className="space-y-4">
              <div className="rounded-xl bg-primary/5 p-4 border border-primary/10">
                <span className="block text-xs font-bold text-foreground/50 uppercase tracking-wider font-mono">30 Days in the Future (+30)</span>
                <span className="block text-lg font-bold text-primary mt-1">{result.futureDate}</span>
              </div>
              <div className="rounded-xl bg-background p-4 border border-border">
                <span className="block text-xs font-bold text-foreground/50 uppercase tracking-wider font-mono">30 Days in the Past (-30)</span>
