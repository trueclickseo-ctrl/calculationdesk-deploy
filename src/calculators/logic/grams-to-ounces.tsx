...
export const seoData = {
  whatIs: 'The Grams to Ounces Converter translates mass between grams and ounces.',
  formula: '$$\\text{Ounces} = \\text{Grams} \\times 0.035274$$',
...
export default function Convertergramstoounces() {
  const [val, setVal] = useState<number>(100);
  const [result, setResult] = useState<number | null>(null);

  const handleReset = () => { setVal(100); setResult(null); };
  const handleCalculate = () => {
    setResult(val * 0.0352739619);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-5 rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-foreground/80 mb-2">Mass in Grams (g)</label>
        ...
        <div className="lg:col-span-7 rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          {result !== null ? (
            <div className="rounded-xl bg-primary/5 p-6 border border-primary/10 text-center">
              ...
              <span className="block text-4xl font-extrabold text-primary mt-2">{result.toFixed(3)} Ounces (oz)</span>
            </div>
          ) : (
