...
export const seoData = {
  whatIs: 'The ML to Cups Converter translates volume from milliliters to standard cooking cups.',
  formula: '$$\\text{Cups} = \\frac{\\text{Milliliters}}{236.588}$$',
...
export default function Convertermltocups() {
  const [val, setVal] = useState<number>(240);
  const [result, setResult] = useState<number | null>(null);

  const handleReset = () => { setVal(240); setResult(null); };
  const handleCalculate = () => {
    setResult(val / 236.588237);
  };
        ...
        <div className="lg:col-span-7 rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          {result !== null ? (
            <div className="rounded-xl bg-primary/5 p-6 border border-primary/10 text-center">
              ...
              <span className="block text-4xl font-extrabold text-primary mt-2">{result.toFixed(2)} US Cups</span>
            </div>
          ) : (
