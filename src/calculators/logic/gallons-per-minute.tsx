...
export const seoData = {
  whatIs: 'Find standard flow discharge velocities in gallons per minute.',
  formula: '$$\\text{Flow (GPM)} = \\text{Velocity (ft/s)} \\times 2.448 \\times \\text{Diameter (in)}^2$$',
  example: 'Velocity of 5 ft/s inside a 2-inch pipe delivers ~49 GPM.',
  faqs: [{ q: 'Is this identical to Pipe Flow Rate?', a: 'Yes, GPM represents the same hydraulic discharge capacity calculation.' }]
};

export default function GallonsPerMinute() {
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-5 rounded-2xl border border-border bg-card p-6 shadow-sm">
          <h2 className="text-xl font-bold text-foreground mb-6">Pipe Flow</h2>
        ...
        <div className="lg:col-span-7 rounded-2xl border border-border bg-card p-6 shadow-sm">
          <h2 className="text-xl font-bold text-foreground mb-6">Discharge Capacity</h2>
          {result !== null ? (
            <div className="rounded-xl bg-primary/5 p-6 border border-primary/10 text-center">
              <span className="block text-xs font-bold text-foreground/50 uppercase font-mono">Discharge GPM</span>
