...
export const seoData = {
  whatIs: `The 90 Day Calculator computes the exact calendar date 90 days in the future or past from any starting date.`,
  formula: `Date offsets are calculated by adding or subtracting days from a start timestamp:

$$\\text{Future Date} = \\text{Start Date} + 90 \\quad (\\text{days})$$
$$\\text{Past Date} = \\text{Start Date} - 90 \\quad (\\text{days})$$`,
  example: `**Find the date 90 days after January 1st:**
...
export default function Days90Calculator() {
  ...
  const handleCalculate = () => {
    ...
    future.setDate(start.getDate() + 90);

    const past = new Date(start);
    past.setDate(start.getDate() - 90);
          ...
          {result ? (
            <div className="space-y-4">
              <div className="rounded-xl bg-primary/5 p-4 border border-primary/10">
                <span className="block text-xs font-bold text-foreground/50 uppercase tracking-wider font-mono">90 Days in the Future (+90)</span>
                <span className="block text-lg font-bold text-primary mt-1">{result.futureDate}</span>
              </div>
              <div className="rounded-xl bg-background p-4 border border-border">
                <span className="block text-xs font-bold text-foreground/50 uppercase tracking-wider font-mono">90 Days in the Past (-90)</span>
